 ...
 * 圖層疊加順序：彩色角色線條在最底 ➔ 中央隊長圖騰（純白實線，T3/T4 外向蔓延）在中間 ➔ 9 個自轉的角色圓徽在最頂層。
 */
...
function 隊長座標轉直角(p: 極座標) {
  const rad = (p.deg * Math.PI) / 180;
  return { x: 畫布中心 + p.r * Math.sin(rad), y: 畫布中心 - p.r * Math.cos(rad) };
}

// 實心純色描邊屬性，全螢幕移除 Glow 濾鏡以保持精美銳利的高對比
    ...
    switch (筆畫.型) {
      ...
      case "圓": {
        ...
        g.appendChild(建立元素("circle", {
          ...
          ...(筆畫.填充
            ? { fill: 顏色, stroke: "none", opacity: 筆畫.淡 ? 0.5 : 1.0 }
            : 描邊屬性(顏色, 1.5, 筆畫.淡)),
        }));
        break;
      }
...
function 渲染隊長Slice(筆畫清單: 圖騰筆畫[], 顏色: string): SVGElement {
  ...
  for (const 筆畫 of 筆畫清單) {
    switch (筆畫.型) {
      case "線": {
        const a = 隊長座標轉直角(筆畫.起);
        const b = 隊長座標轉直角(筆畫.終);
        g.appendChild(建立元素("line", {
          x1: a.x, y1: a.y, x2: b.x, y2: b.y,
          ...描邊屬性(顏色, 筆畫.粗, 筆畫.淡),
        }));
      ...
      case "圓": {
        const c = 隊長座標轉直角(筆畫.中心);
        g.appendChild(建立元素("circle", {
          cx: c.x, cy: c.y, r: 筆畫.半徑,
          ...(筆畫.填充
            ? { fill: 顏色, stroke: "none", opacity: 筆畫.淡 ? 0.55 : 1.0 }
            : 描邊屬性(顏色, 1.5, 筆畫.淡)),
        }));
        break;
      }
...
function 建立編織圖騰SVG(容器: HTMLElement) {
  ...
  // 2. 中下層：建立 內、中、外 3 個旋轉大環組的「萬花筒彩色線條」群組
  const 大環類型列表: 大環類型[] = ["內", "中", "外"];
  大環類型列表.forEach((環key) => {
    const linesGroup = 建立元素("g", { id: `weaving-ring-${環key}-lines` });
    ...
    該大環插槽.forEach((slot) => {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        linesGroup.appendChild(楔形群組);
      }
    });
    svg.appendChild(linesGroup);
  });

  // 3. 中上層：建立中央核心隊長圖騰 (獨立自轉，壓在所有彩色角色線條上方，向外蔓延穿透至 300px)
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const 原始半 = 渲染隊長Slice(隊長筆畫, "#ffffff");
    const 鏡射半 = 建立元素("g", {
      transform: `translate(${畫布尺寸} 0) scale(-1 1)`,
    });
    鏡射半.appendChild(渲染隊長Slice(隊長筆畫, "#ffffff"));

    楔形群組.appendChild(原始半);
    楔形群組.appendChild(鏡射半);
    隊長群組.appendChild(楔形群組);
  }
  svg.appendChild(隊長群組);

  // 4. 最頂層：建立 內、中、外 3 個旋轉大環組的「角色專屬圓徽章」群組 (保證黑底遮擋蓋在最上方)
  大環類型列表.forEach((環key) => {
    const emblemsGroup = 建立元素("g", { id: `weaving-ring-${環key}-emblems` });
    const 該大環插槽 = 已選插槽.filter((s) => s.大環 === 環key);
    ...
    該大環插槽.forEach((slot, idx) => {
      ...
      emblemsGroup.appendChild(emblemCenterG);
    });

    svg.appendChild(emblemsGroup);
  });
...
function startRotationLoop() {
  ...
  function tick() {
    ...
    // 三大環自轉 (同步旋轉 lines 和 emblems)
    const 大環類型列表: 大環類型[] = ["內", "中", "外"];
    大環類型列表.forEach((環key) => {
      ringAngles[環key] += ringSpeeds[環key];
      
      const elLines = document.getElementById(`weaving-ring-${環key}-lines`);
      const elEmblems = document.getElementById(`weaving-ring-${環key}-emblems`);
      
      if (elLines) {
        elLines.setAttribute("transform", `rotate(${ringAngles[環key]} ${畫布中心} ${畫布中心})`);
      }
      if (elEmblems) {
        elEmblems.setAttribute("transform", `rotate(${ringAngles[環key]} ${畫布中心} ${畫布中心})`);
      }
    });
...
function 渲染頁面() {
  ...
  header.innerHTML = `
    ...
    <p>
      <b>3×3 陣列識別系統</b>：外圈成員以 120° 差速旋轉並顯示職責徽章；<b>圓心為專屬隊長六邊形對稱圖騰</b>，純白實線，T3/T4 時向外蔓延穿透，秩序且繁複！
  ...
  capTitle.innerHTML = `<span>👑 中央核心隊長配置 (六邊形印襯萬花筒，帶外蔓延脈絡)</span>`;
  ...
  for (let lv = 1; lv <= 4; lv++) {
    const opt = document.createElement("option");
    opt.value = String(lv);
    opt.textContent = `TIER ` + `I`.repeat(lv) + ` 等級圖騰`;
  ...
  leftPanel.style.maxHeight = "680px";
