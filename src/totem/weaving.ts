 ...
 * 在各層軌道上，均勻分佈三個 (0°, 120°, 240°) 角色專屬身份徽章，無模糊光暈、放大兩倍、發射職責顏色。
 */
...
const 畫布尺寸 = 畫布中心 * 2;
const 作畫最大基準 = 115; // 原先最大半徑 140 下，許多筆畫最大只到 110-120 處，除以 115 可拉伸至外圈邊界，消除脫節
...
// 9層系統半徑配置：大環起點與三色職責發散終點
// 引入 5px 的邊界交疊 (例如內環藍終點 140, 中環起點 135)，徹底防止背景黑圈脫節
...
function 建立編織圖騰SVG(容器: HTMLElement) {
  ...
  // 1. 靜態背景裝飾：每一大環層都有一個「不會自轉的圓形虛線圈」，以清晰標示大環界限與歸屬
  ...
  大環類型列表.forEach((環key) => {
    ...
    // B. 繪製該大環組三個成員的專屬身份圖示。分佈在 0°, 120°, 240°，各佔一個方向。
    // 發射各自職責的發光顏色 (藍/紅/黃)，中央黑底空心，直徑放大兩倍
    該大環插槽.forEach((slot, idx) => {
      ...
      // 擺放在該軌道最外側 84% 處，放大 2 倍後視覺比例完美
      const 圖徽半徑 = cfg.起點 + (終點 - cfg.起點) * 0.84;
      
      // 插槽 1 ➔ 0°, 插槽 2 ➔ 120°, 插槽 3 ➔ 240°，拼成一個三角形均勻分佈
      const 角度 = idx * 120;
      
      // 根據其指派的職責 (藍/紅/黃) 發射專屬顏色
      const 顏色 = 職責配置[slot.職責].顏色;

      // 建立放大兩倍、無光暈濾鏡、白描黑底實心圓的角色身份標識
