 ...
 * 同步支援在各層圓軌上均勻分佈三個 120 度自轉的角色專屬身份圖徽，提高辨識度與美觀度。
 */
...
// ---------------------------------------------------------------------------
// 建立編織 SVG (包含內、中、外 3 個差速旋轉大環組 與 自轉 120度 角色圖徽)
// ---------------------------------------------------------------------------
function 建立編織圖騰SVG(容器: HTMLElement) {
  ...
  大環類型列表.forEach((環key) => {
    ...
    該大環插槽.forEach((slot) => {
      ...
      // 2. 在對應的職責軌道外側掛載「120度均勻分佈、帶有黑底與白描發光的角色身份小圖示」
      const emblemCenterG = 建立元素("g", {});
      // 將中心點移至畫布中央，以便進行 rotate 旋轉定位
      emblemCenterG.setAttribute("transform", `translate(${畫布中心}, ${畫布中心})`);
      ...
      // 定位在該職責發散區段的 82% 外側位置，視覺比例最佳
      const 圖徽半徑 = cfg.起點 + (終點 - cfg.起點) * 0.82;
      
      // 建立白色、黑底中空 120 度自轉身份徽章 (不給予初始偏轉，使其完全隨大環旋轉)
      const glyphGroup = 建立120度圖徽群組(slot.角色.id, slot.等級, 圖徽半徑, "#ffffff", 0);
      emblemCenterG.appendChild(glyphGroup);
      
      // 將圖徽群組塞入大環群組中，實現與萬花筒完美同步的動態差速旋轉！
...
function 渲染頁面() {
  ...
  header.innerHTML = `
    ...
    <p>
      <b>3×3 陣列識別系統</b>：除了美麗的萬花筒線條，每個角色均在圓軌上擁有三個 120° 均勻分布、黑底白線的<b>專屬身份圖徽</b>，大幅提升戰鬥辨識度！
      <br/>可在此點選並指派成員大環、職責顏色（藍/紅/黃）與星級等級（1★~3★）。
  ...
  全體成員.forEach((m) => {
    ...
    if (!isSelected) {
      item.addEventListener("click", () => {
        ...
        已選插槽.push({ 角色: m, 大環: 當前指派環, 職責: 預設職責, 等級: 3 }); // 預設 3★
        更新模擬器();
      });
    }
...
// 預設載入一個華麗的 9 人陣容 Demo，並交錯各星等星級展示圖示效果
已選插槽 = [
  // 內環組 3人 (幾何/有機)
  { 角色: 全體成員[0], 大環: "內", 職責: "藍", 等級: 1 }, // 稜鏡 (皇冠 1★)
  { 角色: 全體成員[2], 大環: "內", 職責: "紅", 等級: 2 }, // 向量 (衝鋒箭 2★)
  { 角色: 全體成員[3], 大環: "內", 職責: "黃", 等級: 3 }, // 節點 (十字網格 3★)

  // 中環組 3人 (分形)
  { 角色: 全體成員[10], 大環: "中", 職責: "藍", 等級: 3 }, // 雪鏡 (雪花鏡環 3★)
  { 角色: 全體成員[12], 大環: "中", 職責: "紅", 等級: 1 }, // 閃電 (單閃電Z 1★)
  { 角色: 全體成員[14], 大環: "中", 職責: "黃", 等級: 2 }, // 極光 (雙軌正弦 2★)

  // 外環組 3人 (機械)
  { 角色: 全體成員[15], 大環: "外", 職責: "藍", 等級: 2 }, // 閘門 (十字焊接門 2★)
  { 角色: 全體成員[17], 大環: "外", 職責: "紅", 等級: 3 }, // 鋼針 (十字衝擊針 3★)
  { 角色: 全體成員[18], 大環: "外", 職責: "黃", 等級: 1 }, // 發條 (螺旋發條 1★)
];
