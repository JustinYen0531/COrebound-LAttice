...
const 隊長立繪路徑: Record<string, string> = {
  conductor: "/transparent-portraits/Conductor立繪與頭像.png",
  operator: "/transparent-portraits/Operator立繪與頭像.png",
  launcher: "/transparent-portraits/Launcher立繪與頭像.png",
  architect: "/transparent-portraits/Architect立繪與頭像.png",
};
...
function 建立成員立繪HTML(條目ID: string): string {
  ...
  const 立繪路徑 = `/transparent-portraits/members/${m.id}_s${選中星級}.png`;
...
function 建立怪物立繪HTML(條目ID: string): string {
  ...
  return 建立圖鑑舞台立繪HTML(
    `/images/enemies/${m.world}/${m.id}.png`,
    `${m.nameZh} 立繪`
  );
}

// 隊長條目 id 格式 "captain_{captainId}"。此分支優先直接使用原專案現成的單張去背立繪。
function 建立隊長立繪HTML(條目ID: string): string {
  if (!條目ID.startsWith("captain_")) return "";
  const captainId = 條目ID.replace("captain_", "");
  const 立繪路徑 = 隊長立繪路徑[captainId];
  if (!立繪路徑) return "";
