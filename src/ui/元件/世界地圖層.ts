...
// 怪物 token 使用去背 PNG；Tier 明確拉開體型，讓 T1 < T2 < T3 的威脅層級一眼可讀。
...
export function 建立世界地圖層(): HTMLElement {
  ...
  function setCameraZoom(nextZoom: number): void {
    ...
    for (const m of monsters) {
      const baseSize = monsterSizeForTier(m.inst.tier);
...
function createMonsterNode(inst: 可見怪物實例): HTMLElement {
  ...
  const baseSize = monsterSizeForTier(inst.tier);
...
function monsterSizeForTier(tier: EnemyTier): number {
  return MONSTER_SIZE_AT_REFERENCE_ZOOM[tier] ?? MONSTER_SIZE_AT_REFERENCE_ZOOM[1];
}
